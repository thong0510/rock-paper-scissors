...
const imageSrc = { rock: rockImg, paper: paperImg, scissors: scissorsImg };
...
const ChoiceCard = ({ title, winner, shape }) => {
  return (
    ...
    >
      ...
      {winner === -1 && <h2 className="fw-bold text-danger">Lost</h2>}
      {winner === 0 && <h2 className="fw-bold">Tie</h2>}
      {winner === 1 && <h2 className="fw-bold text-success">Won</h2>}
    </div>
  );
};
